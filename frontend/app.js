...
const els = {
    ...
    codeBlock: byId("codeBlock"),
    codeTag: byId("codeTag"),
    codeMeta: byId("codeMeta"),
    codePanel: bySelector(".code-panel"),
...
function handleTelemetry(msg) {
    ...
    const tempPanel = bySelector('[data-sensor="temperature"]');
    ...
    if (stats) {
        if (stats.temp_variance !== undefined)
            els.statT.textContent = stats.temp_variance.toFixed(4);
        if (stats.pressure_variance !== undefined)
            els.statP.textContent = stats.pressure_variance.toFixed(2);
        if (stats.current_variance !== undefined)
            els.statI.textContent = stats.current_variance.toFixed(4);
    }
...
function handleAnomaly(msg) {
    ...
    bySelector('[data-sensor="temperature"]').classList.add("alert");
...
function handleStatus(msg) {
    ...
    if (msg.status === "monitoring") {
        bySelector('[data-sensor="temperature"]').classList.remove("alert");
    }
}
let typeAnimToken = 0;
