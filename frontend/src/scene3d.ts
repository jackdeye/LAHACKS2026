...
export type PartKey =
  | "uno" | "flipper" | "lcd"
  | "breadboard" | "dht11" | "pot" | "pressure" | "current" | "motor" | "fan";

export interface SceneApi {
  setStatus(label: string): void;
  parts: Partial<Record<PartKey, THREE.Object3D>>;
  setSensorFailure(sensorId: PartKey, isFailing: boolean): void;
}

interface GlbEntry {
  url: string;
  longestDimCm: number;     // real-world longest dimension
  position: [number, number, number];
  rotation: [number, number, number];
}

const GLBS: Record<"uno" | "flipper" | "lcd", GlbEntry> = {
  uno: {
...
// ── Procedural parts ───────────────────────────────────────────────────────
function makeBreadboard(): THREE.Group {
  ...
  const body = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({ color: 0xe8e6dc, roughness: 0.7, metalness: 0.05 }),
  );
  body.position.y = H / 2;
  g.add(body);
  ...
  const top = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({ map: tex, roughness: 0.8, metalness: 0.04 }),
  );
...
function makeDHT11(): THREE.Group {
  const g = new THREE.Group();
  // PCB
...
function makePot(): THREE.Group {
  ...
  const shaft = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({ color: 0xc0c0c0, roughness: 0.4, metalness: 0.9 }),
  );
  ...
  const mark = new THREE.Mesh(
    new THREE.BoxGeometry(0.45, 0.04, 0.08),
    new THREE.MeshBasicMaterial({ color: 0xff3e5f }),
  );
...
function makeBreakoutPcb(
  width: number,
  depth: number,
  pcbColor: number,
  pinCount: number,
): { group: THREE.Group; pcbMat: THREE.MeshStandardMaterial } {
  const g = new THREE.Group();
...
function makePressure(): THREE.Group {
...
function makeCurrent(): THREE.Group {
...
function makeMotorFan(): { group: THREE.Group; fan: THREE.Object3D } {
  const g = new THREE.Group();
  // motor body (silver cylinder lying on its side along Z)
  const body = new THREE.Mesh(
    new THREE.CylinderGeometry(0.7, 0.7, 2.0, 24),
    new THREE.MeshStandardMaterial({ color: 0xb8b8b8, roughness: 0.45, metalness: 0.85 }),
  );
  ...
  const shaft = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({ color: 0x999999, roughness: 0.3, metalness: 0.95 }),
  );
  ...
  const hub = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({ color: 0x222a32, roughness: 0.6, metalness: 0.3 }),
  );
...
export async function initScene(
  ...
  const parts: Partial<Record<PartKey, THREE.Object3D>> = {};
  ...
  function makeHalo(): THREE.Mesh {
    const halo = new THREE.Mesh(
      new THREE.RingGeometry(0.9, 1.8, 48),
  ...
  function makeFailLight(): THREE.PointLight {
  ...
  async function loadGlb(entry: GlbEntry, label: string): Promise<THREE.Object3D> {
  ...
  const bbRow = (x: number, z: number): THREE.Vector3 =>
    new THREE.Vector3(x, 0.9, z);

  type WireSpec = { from: THREE.Vector3; to: THREE.Vector3; color: number; sag?: number };
  ...
  const failingSensors = new Set<PartKey>();
  ...
  const ANOMALY_SENSORS: PartKey[] = ["dht11", "pot", "pressure", "current"];
  ...
  function tick(): void {
    ...
    ANOMALY_SENSORS.forEach((id) => {
      ...
      const mats = (part.userData.glowMats as THREE.MeshStandardMaterial[] | undefined) ?? [];
      for (const m of mats) {
        if (failing) {
      ...
      // (2) Ground halo
      const halo = part.userData.halo as THREE.Mesh | undefined;
      if (halo) {
      ...
      const light = part.userData.failLight as THREE.PointLight | undefined;
      if (light) {
  ...
  return {
    setStatus(label: string): void { onProgress(label); },
    parts,
    setSensorFailure(sensorId: PartKey, isFailing: boolean): void {
      if (isFailing) {
